...
def import_dtsanphamthanhlong_process(imgurl):
      # Process files here
    ...
    for sheet in wb:
      ...
      #   print(fw1)
        ...
        #đổi tên cột
        df12=df11.rename(columns={'0': 'tenhuyen','1': 'nam','2':'dt_sanpham'})
        ...
        #đổi tên cột
        df14=df13.rename(columns={'0': 'tenhuyen','1': 'nam','2':'dt_sanpham'})
        #thêm cột
      ...
      #   df13.drop(df13[df13['nam'] == '0'].index, inplace = True)
        ...
        df14.to_sql(name='dt_sanpham_thanhlong',con=engine, schema = 'import_data',if_exists='append',index=False)
    sql = text('update import_data.dt_sanpham_thanhlong Set maso=MD5(id::text || RANDOM()::text || CURRENT_TIMESTAMP::text) where maso is Null ;')
